...
void updatePixel(int x, int y, char character, uint8_t color) {
  if (x >= 0 && x < numCols && y >= 0 && y < numRows) {
    pixelArray[x][y].character = character;
    pixelArray[x][y].color = color;
  }
}
...
void processSerialInput() {
  if (Serial.available()) {
    ...
    if (input == 'X') {
      if (Serial.available()) {
        currentX = Serial.parseInt();
        currentX = constrain(currentX, 0, numCols - 1);
      }
    } else if (input == 'Y') {
      if (Serial.available()) {
        currentY = Serial.parseInt();
        currentY = constrain(currentY, 0, numRows - 1);
      }
    } else if (input == 'C') {
      if (Serial.available()) {
        ...
        if (Serial.available()) {
          ...
          updatePixel(currentX, currentY, character, color);
        }
      }
    }
  }
